...
fn main() {
    sdkwork_aiot_cloud_gateway::assert_production_environment_safety();
    if sdkwork_aiot_cloud_gateway::dev_mode_enabled() {
        eprintln!(
    ...
    let bridge_state = sdkwork_aiot_cloud_gateway::MqttBridgeRuntimeState::from_env();
    let (server, session_options) =
        sdkwork_aiot_cloud_gateway::standard_gateway_server_and_session_options()
            .map(|(server, session_options)| {
                (
                    sdkwork_aiot_cloud_gateway::attach_gateway_readiness_probe(
                        server,
    ...
    println!(
        "sdkwork-aiot-cloud-gateway mode={:?} components={} xiaozhi_websocket={}",
        server.runtime.mode(),
    ...
    sdkwork_aiot_cloud_gateway::start_outbox_dispatcher_worker(
        Arc::clone(&running),
        Arc::clone(&outbox_lag),
    );
    ...
    println!("sdkwork-aiot-cloud-gateway listening on http://{bind_addr}");
    ...
    sdkwork_aiot_observability::emit_runtime_metric(
        &sdkwork_aiot_observability::RuntimeMetricFields::new("sdkwork-aiot-cloud-gateway")
            .connections(0),
    );
    ...
    while running.load(Ordering::Relaxed) {
        let stream = match listener.accept() {
            ...
            Err(error) => {
                eprintln!("sdkwork-aiot-cloud-gateway accept_error={error}");
                std::thread::sleep(Duration::from_millis(50));
                continue;
            }
        };
    ...
    fn from_env() -> Self {
        Self {
            ...
            client_id: std::env::var("SDKWORK_AIOT_GATEWAY_MQTT_CLIENT_ID")
                .unwrap_or_else(|_| "sdkwork-aiot-cloud-gateway".to_string()),
    ...
    fn publish_reply(
        ...
        let Some(client) = state.client.as_ref() else {
            if !reply.outbound_json.is_empty() {
                eprintln!("sdkwork-aiot-cloud-gateway mqtt_publish_skipped=no_active_client");
            }
            return;
        };
...
fn apply_bridge_session_state_locked(
    ...
    reply: &sdkwork_aiot_cloud_gateway::MqttSessionReply,
    next_session: Option<sdkwork_aiot_cloud_gateway::XiaozhiMqttUdpSession>,
) {
...
fn send_bridge_udp_packets(
    ...
    let Some(peer) = peer else {
        if !packets.is_empty() {
            eprintln!("sdkwork-aiot-cloud-gateway udp_send_skipped=no_session_peer");
        }
        return;
    };
    for packet in packets {
        if let Err(error) = udp_socket.send_to(packet, peer) {
            eprintln!("sdkwork-aiot-cloud-gateway udp_send_error={error}");
        } else {
...
fn publish_bridge_json_messages(
    ...
    for outbound in bounded_outbound {
        ...
        if let Err(error) = publish_with_retry(
            ...
            eprintln!("sdkwork-aiot-cloud-gateway mqtt_publish_error={error}");
...
impl BridgeStats {
    ...
    fn maybe_log_snapshot(&self, interval: Duration) {
        ...
        eprintln!(
            "sdkwork-aiot-cloud-gateway bridge_stats reconnects={} mqtt_events={} mqtt_event_errors={} mqtt_session_errors={} publish_attempts={} publish_failures={} publish_retries={} publish_dropped={} udp_packets={} udp_decode_failures={} session_idle_purges={} udp_uplink_replies={}",
...
fn start_mqtt_udp_bridge(
    ...
    session_options: sdkwork_aiot_cloud_gateway::XiaozhiSessionOptions,
    running: Arc<AtomicBool>,
    stats: Arc<BridgeStats>,
    state: Arc<sdkwork_aiot_cloud_gateway::MqttBridgeRuntimeState>,
    ...
    let udp_socket = match UdpSocket::bind(&udp_config.bind_addr) {
        ...
        Err(error) => {
            eprintln!("sdkwork-aiot-cloud-gateway udp_bind_error={error}");
            return Arc::clone(&bridge_control);
        }
    };
    if let Err(error) = udp_socket.set_read_timeout(Some(udp_config.read_timeout)) {
        eprintln!("sdkwork-aiot-cloud-gateway udp_timeout_error={error}");
        return Arc::clone(&bridge_control);
    }
...
fn run_mqtt_bridge_loop(
    ...
    while running.load(Ordering::Relaxed) {
        ...
        if let Err(error) = client.subscribe(config.subscribe_topic.clone(), QoS::AtMostOnce) {
            ...
            eprintln!("sdkwork-aiot-cloud-gateway mqtt_subscribe_error={error}");
        ...
        loop {
            let event = match connection.iter().next() {
                ...
                Some(Err(error)) => {
                    ...
                    eprintln!("sdkwork-aiot-cloud-gateway mqtt_event_error={error}");
                    break;
                }
                None => {
                    eprintln!("sdkwork-aiot-cloud-gateway mqtt_event_stream_closed");
                    break;
                }
            };
            ...
            if let Event::Incoming(Incoming::Publish(publish)) = event {
                ...
                let response = sdkwork_aiot_cloud_gateway::xiaozhi_mqtt_session_reply_with_options(
                    &server,
...
fn run_udp_bridge_loop(
    ...
    while running.load(Ordering::Relaxed) {
        ...
        let (len, from) = match udp_socket.recv_from(&mut buf) {
            ...
            Err(error) => {
                eprintln!("sdkwork-aiot-cloud-gateway udp_recv_error={error}");
                continue;
            }
        };
        stats.udp_packets_total.fetch_add(1, Ordering::Relaxed);
        ...
        struct UdpUplinkReplyAction {
            session_key: String,
            reply: sdkwork_aiot_cloud_gateway::MqttSessionReply,
            session: sdkwork_aiot_cloud_gateway::XiaozhiMqttUdpSession,
            peer: SocketAddr,
        }
        ...
        let mut uplink_action = None;
        {
            ...
            for (session_key, managed) in guard.sessions.iter_mut() {
                let packet = match sdkwork_aiot_cloud_gateway::xiaozhi_mqtt_udp_decode_audio(
                    &managed.session,
                    &buf[..len],
                ) {
                    Ok(packet) => packet,
                ...
                match sdkwork_aiot_cloud_gateway::xiaozhi_mqtt_udp_uplink_speech_reply(
                    &mut session,
                    &packet.payload,
                ) {
                    Ok(reply) => {
...
fn install_ctrlc_handler(running: Arc<AtomicBool>) {
    if let Err(error) = ctrlc::set_handler(move || {
        ...
        eprintln!("sdkwork-aiot-cloud-gateway ctrlc_handler_error={error}");
    }
}

struct ClientConnectionContext {
    bridge_stats: Arc<BridgeStats>,
    bridge_state: Arc<sdkwork_aiot_cloud_gateway::MqttBridgeRuntimeState>,
...
fn handle_client_connection(
    ...
    let buffer = match sdkwork_aiot_transport::read_full_http_request(
        ...
        Ok(buffer) => buffer,
        Err(error) => {
            eprintln!("sdkwork-aiot-cloud-gateway read_error={}", error.code);
    ...
    if let Ok((request, header_len)) = parsed_request {
        if request.path.starts_with("/internal/")
            && !sdkwork_aiot_cloud_gateway::internal_route_authorized(&request)
        {
        ...
        {
            if !sdkwork_aiot_cloud_gateway::xiaozhi_device_token_valid(&request, session_options) {
            ...
            if sdkwork_aiot_cloud_gateway::ws_backpressure_rejects_new_connection(
                &server.runtime,
    ...
    if let Err(error) = stream.write_all(response.as_bytes()) {
        eprintln!("sdkwork-aiot-cloud-gateway write_error={error}");
    }
}
...
fn handle_xiaozhi_websocket_session(
    ...
    loop {
        ...
        let read = match stream.read(&mut read_buffer) {
            ...
            Ok(read) => read,
            Err(error) => {
                eprintln!("sdkwork-aiot-cloud-gateway websocket_read_error={error}");
                return;
            }
        };
        frame_buffer.extend_from_slice(&read_buffer[..read]);
        if frame_buffer.len() > MAX_WEBSOCKET_FRAME_BUFFER_BYTES {
            eprintln!("sdkwork-aiot-cloud-gateway websocket_frame_buffer_limit_exceeded");
            return;
        }
    }
}
...
fn process_xiaozhi_frame_buffer(
    ...
    loop {
        ...
        let replies = match sdkwork_aiot_cloud_gateway::xiaozhi_websocket_session_reply_with_options(
            server,
            request,
            frame,
            session_options,
...
fn bridge_health_response(
    runtime_state: &sdkwork_aiot_cloud_gateway::MqttBridgeRuntimeState,
    stats: &BridgeStats,
...
fn bridge_metrics_response(stats: &BridgeStats) -> sdkwork_aiot_transport::HttpResponse {
    ...
    let activation_registry_snapshot =
        sdkwork_aiot_cloud_gateway::xiaozhi_activation_registry_stats_snapshot();
...
fn activation_registry_stats_response() -> sdkwork_aiot_transport::HttpResponse {
    let snapshot = sdkwork_aiot_cloud_gateway::xiaozhi_activation_registry_stats_snapshot();
    let body = format!(
...
fn activation_registry_prometheus_text(
    snapshot: &sdkwork_aiot_cloud_gateway::XiaozhiActivationRegistryStatsSnapshot,
) -> String {
...
fn bridge_health_snapshot_json(
    runtime: &sdkwork_aiot_cloud_gateway::MqttBridgeRuntimeSnapshot,
    stats: &BridgeStatsSnapshot,
) -> String {
    let status = sdkwork_aiot_cloud_gateway::mqtt_bridge_health_status(runtime);
...
mod tests {
    ...
    fn bridge_health_snapshot_json_reports_runtime_status_and_stats() {
        let runtime = sdkwork_aiot_cloud_gateway::MqttBridgeRuntimeSnapshot {
            bridge_enabled: true,
    ...
    fn apply_bridge_session_state_removes_session_when_channel_closes() {
        ...
        let reply = sdkwork_aiot_cloud_gateway::xiaozhi_mqtt_server_teardown_reply(&session_key);
