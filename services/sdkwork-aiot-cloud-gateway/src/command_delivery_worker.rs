...
use crate::active_ws_sessions::{
    active_ws_device_ids, evict_stale_active_ws_sessions, push_ws_command_replies,
};
use crate::xiaozhi_speak_websocket_replies;
    ...
    std::thread::spawn(move || {
        while running.load(Ordering::Relaxed) {
            if let Err(error) = run_command_delivery_once(session_ttl) {
                eprintln!("sdkwork-aiot-cloud-gateway command_delivery_error={error}");
            }
...
fn run_command_delivery_once(session_ttl: Duration) -> Result<(), String> {
    ...
    for device_id in online_devices {
        ...
        for delivery in pending {
            ...
            let replies = xiaozhi_speak_websocket_replies(&request, &session_id, &text)
