...
const SIMULATOR_SERVER_NAME: &str = "sdkwork-aiot-cloud-gateway";
...
impl SqliteXiaozhiActivationChallengeRegistry {
    pub fn try_new(path: PathBuf) -> Result<Self, String> {
        ...
        if let Err(error) = registry.ensure_schema() {
            eprintln!(
                "sdkwork-aiot-cloud-gateway activation_registry_sqlite_schema_error path={} error={error}",
...
impl XiaozhiActivationChallengeRegistry for FileBackedXiaozhiActivationChallengeRegistry {
    fn register_challenge(&self, request: &HttpRequest, challenge: &str, timeout_ms: u32) {
        ...
        match result {
            ...
            Err(error) => {
                eprintln!(
                    "sdkwork-aiot-cloud-gateway activation_registry_register_error path={} error={error}",
                    self.path.display()
                );
            }
        }
    ...
    fn consume_challenge(&self, request: &HttpRequest, challenge: &str) -> bool {
        ...
        match self.with_locked_disk_entries(|entries| {
            ...
            Err(error) => {
                eprintln!(
                    "sdkwork-aiot-cloud-gateway activation_registry_consume_error path={} error={error}",
                    self.path.display()
                );
...
DO UPDATE SET expires_at_millis=excluded.expires_at_millis",
        ...
        match result {
            ...
            Err(error) => {
                eprintln!(
                    "sdkwork-aiot-cloud-gateway activation_registry_sqlite_register_error path={} error={error}",
                    self.path.display()
                );
            }
        }
...
WHERE device_id=?1 AND client_id=?2 AND challenge=?3 AND expires_at_millis > ?4",
        ...
        match result {
            ...
            Err(error) => {
                eprintln!(
                    "sdkwork-aiot-cloud-gateway activation_registry_sqlite_consume_error path={} error={error}",
                    self.path.display()
                );
                activation_registry_record_consume_outcome(false);
                false
...
impl XiaozhiActivationChallengeRegistry for RedisXiaozhiActivationChallengeRegistry {
    fn register_challenge(&self, request: &HttpRequest, challenge: &str, timeout_ms: u32) {
        ...
        let mut conn = match self.open_connection() {
            ...
            Err(error) => {
                eprintln!(
                    "sdkwork-aiot-cloud-gateway activation_registry_redis_connect_error url={} error={error}",
                    self.redis_url
                );
                return;
            }
        ...
        if let Err(error) = result {
            eprintln!(
                "sdkwork-aiot-cloud-gateway activation_registry_redis_register_error url={} key={} error={error}",
                self.redis_url, key
            );
        }
    }
...
return 0
        ...
        let deleted = match script.key(&key).invoke::<u64>(&mut conn) {
            ...
            Err(error) => {
                eprintln!(
                    "sdkwork-aiot-cloud-gateway activation_registry_redis_consume_error url={} key={} error={error}",
                    self.redis_url, key
                );
                activation_registry_record_consume_outcome(false);
                return false;
...
fn log_protocol_ingest_receipt(receipt: &sdkwork_aiot_storage::AiotStorageWriteReceipt) {
    ...
    eprintln!(
        "sdkwork-aiot-cloud-gateway protocol_ingest_error accepted={} duplicate={} reason={:?}",
...
fn finalize_protocol_ingest(
    ...
    if let Err(error) = catalog.mark_offered_deployment_completed_for_device(device_id) {
        if !matches!(error, SqlitePersistedEntityError::NotFound) {
            eprintln!(
                "sdkwork-aiot-cloud-gateway firmware_deployment_complete_error device_id={device_id} error={error:?}"
            );
        }
    }
}
...
fn mcp_policy_decision_log_line(
    ...
    format!(
        "sdkwork-aiot-cloud-gateway mcp_policy_decision decision={} rule_index={} transport={} session_id={} device_id={} client_id={} tool={} arguments_present={} message=\"{}\"",
        decision,
        rule_index,
        context.transport,
        context.session_id,
...
fn activation_challenge_registry_from_env() -> Arc<dyn XiaozhiActivationChallengeRegistry> {
        ...
        .is_some_and(|value| value.eq_ignore_ascii_case("redis"))
    {
        ...
        eprintln!(
            "sdkwork-aiot-cloud-gateway activation_registry_redis_missing_url kind=redis fallback=in_memory"
        );
    ...
    if let Some(path) = env_string(ENV_XIAOZHI_ACTIVATION_REGISTRY_PATH) {
            ...
            .is_some_and(|value| value.eq_ignore_ascii_case("sqlite"))
        {
            match SqliteXiaozhiActivationChallengeRegistry::try_new(PathBuf::from(path.clone())) {
                ...
                Err(error) => {
                    eprintln!(
                        "sdkwork-aiot-cloud-gateway activation_registry_sqlite_open_error path={path} error={error} fallback=file"
                    );
                }
            }
        }
