> Migrated from `docs/superpowers/plans/2026-05-31-sdkwork-aiot-server.md` on 2026-06-24.
...
- Create `services/sdkwork-aiot-device-edge-runtime`: standalone gateway shell using runtime builder.
...
**Files:**
- Modify: `services/sdkwork-aiot-device-edge-runtime/src/main.rs`
...
- [x] Updated `services/sdkwork-aiot-device-edge-runtime` to start a standalone TCP HTTP gateway by default while still allowing `SDKWORK_AIOT_DEVICE_EDGE_NO_LISTEN=1` for startup checks.
- [x] Verified `sdkwork-aiot-device-edge-runtime.exe` starts on a temporary port and returns `/healthz` with `ready=true`, then stops cleanly.
