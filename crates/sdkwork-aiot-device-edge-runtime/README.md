...
Domain: iot
Capability: device-edge-ingress
Package type: Rust device protocol runtime
Status: standard

This README is the SDKWork module entrypoint for `sdkwork-aiot-device-edge-runtime`. The machine-readable component contract is `specs/component.spec.json`; canonical standards are under `../../../sdkwork-specs/`.

This process owns WebSocket, MQTT, UDP, OTA, activation, media-session, and
device-command delivery behavior. It does not own app-api, backend-api,
open-api, an application standalone gateway, or the platform cloud gateway.
