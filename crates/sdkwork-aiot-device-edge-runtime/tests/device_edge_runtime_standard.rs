...
use sdkwork_aiot_device_edge_runtime::{
    ...
    xiaozhi_mqtt_udp_uplink_speech_reply, xiaozhi_websocket_session_reply,
    xiaozhi_websocket_session_reply_with_mcp_tool_provider, DefaultXiaozhiActivationVerifier,
    DefaultXiaozhiSimulatorMcpToolProvider, InMemoryXiaozhiActivationChallengeRegistry,
    RolloutAwareXiaozhiOtaProfileProvider, WebSocketSessionReply, XiaozhiActivationVerifier,
...
impl sdkwork_aiot_device_edge_runtime::XiaozhiSimulatorMcpToolInvoker for ErroringToolInvoker {
    fn invoke(
        &self,
        _context: &sdkwork_aiot_device_edge_runtime::XiaozhiMcpInvocationContext,
        _tool: &sdkwork_aiot_device_edge_runtime::XiaozhiSimulatorMcpToolSpec,
...
impl sdkwork_aiot_device_edge_runtime::XiaozhiSimulatorMcpToolInvoker for ContextEchoToolInvoker {
    fn invoke(
        &self,
        context: &sdkwork_aiot_device_edge_runtime::XiaozhiMcpInvocationContext,
        _tool: &sdkwork_aiot_device_edge_runtime::XiaozhiSimulatorMcpToolSpec,
        _tool_arguments_json: Option<&str>,
    ) -> Result<String, String> {
        Ok(format!(
...
impl sdkwork_aiot_device_edge_runtime::XiaozhiSimulatorMcpToolPolicy for DenyAllToolPolicy {
    fn allow(
        &self,
        _context: &sdkwork_aiot_device_edge_runtime::XiaozhiMcpInvocationContext,
        _tool: &sdkwork_aiot_device_edge_runtime::XiaozhiSimulatorMcpToolSpec,
...
#[test]
fn standard_device_edge_server_mounts_xiaozhi_ota_compatibility_route() {
    let server = standard_device_edge_server().expect("device edge server");
...
#[test]
fn standard_device_edge_server_uses_plain_ws_for_local_xiaozhi_ota_debugging() {
    let server = standard_device_edge_server().expect("device edge server");
...
#[test]
fn standard_device_edge_server_can_publish_udp_crypto_profile_in_ota_response() {
...
#[test]
fn standard_device_edge_server_supports_pluggable_activation_verifier_and_ota_provider() {
    #[derive(Debug)]
    struct TestVerifier;
    ...
    let server =
        standard_device_edge_server_with_plugins(Arc::new(TestOtaProvider), Arc::new(TestVerifier))
            .expect("device edge server");
...
#[test]
fn standard_device_edge_server_mounts_xiaozhi_activation_pending_route() {
    let server = standard_device_edge_server().expect("device edge server");
...
#[test]
fn standard_device_edge_server_mounts_xiaozhi_ota_activate_alias_route() {
    let server = standard_device_edge_server().expect("device edge server");
...
#[test]
fn standard_device_edge_server_accepts_activation_with_issued_challenge_and_rejects_replay() {
    let _guard = EnvGuard::set_all_locked(&[
        (
...
#[test]
fn standard_device_edge_server_strict_v2_activation_requires_algorithm_and_serial_alignment() {
    let _guard = EnvGuard::set_all_locked(&[
        (
            "SDKWORK_AIOT_XIAOZHI_ACTIVATION_CHALLENGE",
            Some("challenge-v2"),
...
#[test]
fn standard_device_edge_server_strict_v2_activation_keeps_legacy_non_v2_path() {
    let _guard = EnvGuard::set_all_locked(&[
        (
            "SDKWORK_AIOT_XIAOZHI_ACTIVATION_CHALLENGE",
...
#[test]
fn standard_device_edge_server_rejects_expired_activation_challenge() {
    let _guard = EnvGuard::set_all_locked(&[
        (
            "SDKWORK_AIOT_XIAOZHI_ACTIVATION_CHALLENGE",
...
#[test]
fn standard_device_edge_server_persists_activation_challenge_when_registry_path_is_configured() {
    ...
    let server_before_restart =
        standard_device_edge_server().expect("device edge server before restart");
    let ota = handle_http_request_bytes(
    ...
    let server_after_restart =
        standard_device_edge_server().expect("device edge server after restart");
    let activation = handle_http_request_bytes(
...
fn file_backed_activation_registry_keeps_state_across_two_concurrent_servers() {
    ...
    let server_a = standard_device_edge_server().expect("device edge server a");
    let server_b = standard_device_edge_server().expect("device edge server b");
...
#[test]
fn standard_device_edge_server_builder_accepts_injected_mcp_tool_provider() {
    #[derive(Debug)]
    struct TestOtaProvider;
    impl XiaozhiOtaProfileProvider for TestOtaProvider {
        fn enrich(
    ...
    let server = standard_device_edge_server_with_plugins_activation_registry_and_mcp_tools(
        ...
        Arc::new(DefaultXiaozhiSimulatorMcpToolProvider::from_tools(vec![
            sdkwork_aiot_device_edge_runtime::XiaozhiSimulatorMcpToolSpec::new(
                "self.custom.ping",
...
fn xiaozhi_websocket_session_reply_with_options_uses_injected_provider() {
    ...
    let replies = sdkwork_aiot_device_edge_runtime::xiaozhi_websocket_session_reply_with_options(
        &server,
        &request,
        sdkwork_aiot_transport::WebSocketFrame::text(
...
#[test]
fn standard_device_edge_server_and_session_options_builder_propagates_mcp_provider() {
    #[derive(Debug)]
    struct TestOtaProvider;
    impl XiaozhiOtaProfileProvider for TestOtaProvider {
        fn enrich(
    ...
    let (server, options) =
        ...
        .expect("device edge server with options");
...
#[test]
fn standard_device_edge_server_enforces_xiaozhi_activation_post_method() {
    let server = standard_device_edge_server().expect("device edge server");
    ...
    assert!(response.contains("iot.xiaozhi.activate.method"));
}
...
fn xiaozhi_mqtt_session_reply_with_options_persists_protocol_storage_command() {
    ...
    let (reply, _session) = sdkwork_aiot_device_edge_runtime::xiaozhi_mqtt_session_reply_with_options(
        &server,
        None,
...
fn xiaozhi_websocket_session_mcp_initialize_returns_protocol_metadata() {
    ...
    assert!(text.iter().any(|value| value.contains(
        r#""serverInfo":{"name":"sdkwork-aiot-device-edge-runtime","version":"0.1.0"}"#
    )));
}
...
fn xiaozhi_mqtt_session_reply_with_options_uses_injected_provider() {
    ...
    let (reply, next_session) = sdkwork_aiot_device_edge_runtime::xiaozhi_mqtt_session_reply_with_options(
        &server,
        Some(&session),
...
#[test]
fn device_edge_serves_bridge_health_endpoint() {
    let bind_addr = reserve_local_bind_addr();
    let _edge_runtime = DeviceEdgeProcess::spawn(&bind_addr);

    wait_for_device_edge(&bind_addr, Duration::from_secs(10));
...
#[test]
fn device_edge_serves_bridge_sessions_endpoint_when_bridge_is_disabled() {
    let bind_addr = reserve_local_bind_addr();
    let _edge_runtime = DeviceEdgeProcess::spawn(&bind_addr);
...
#[test]
fn device_edge_bridge_session_disconnect_returns_disabled_when_bridge_is_off() {
    let bind_addr = reserve_local_bind_addr();
    let _edge_runtime = DeviceEdgeProcess::spawn(&bind_addr);
    ...
    assert!(response.contains("iot.device-edge.bridge.disabled"));
}

#[test]
fn device_edge_bridge_session_disconnect_returns_not_found_when_bridge_is_enabled() {
    let bind_addr = reserve_local_bind_addr();
    let _edge_runtime = DeviceEdgeProcess::spawn_with_env(
        &bind_addr,
        &[("SDKWORK_AIOT_DEVICE_EDGE_MQTT_BRIDGE_ENABLE", Some("1"))],
    );
    ...
    assert!(response.contains("iot.device-edge.bridge.session.not_found"));
}

#[test]
fn device_edge_serves_bridge_metrics_endpoint_with_activation_registry_metrics() {
    let bind_addr = reserve_local_bind_addr();
    let _edge_runtime = DeviceEdgeProcess::spawn(&bind_addr);
...
#[test]
fn device_edge_serves_xiaozhi_mcp_policy_stats_endpoint() {
    let bind_addr = reserve_local_bind_addr();
    let _edge_runtime = DeviceEdgeProcess::spawn(&bind_addr);
...
#[test]
fn device_edge_serves_xiaozhi_activation_registry_stats_endpoint() {
    let bind_addr = reserve_local_bind_addr();
    let _edge_runtime = DeviceEdgeProcess::spawn(&bind_addr);
...
#[test]
fn device_edge_serves_xiaozhi_activation_registry_metrics_endpoint() {
    let bind_addr = reserve_local_bind_addr();
    let _edge_runtime = DeviceEdgeProcess::spawn(&bind_addr);
...
#[test]
fn device_edge_activation_registry_metrics_reflect_in_memory_backend_after_activation_flow() {
    let bind_addr = reserve_local_bind_addr();
    let _edge_runtime = DeviceEdgeProcess::spawn_with_env(
        &bind_addr,
        &[
...
#[test]
fn device_edge_activation_registry_metrics_reflect_file_backend_after_activation_flow() {
    let bind_addr = reserve_local_bind_addr();
...
#[test]
fn device_edge_activation_registry_metrics_reflect_sqlite_backend_after_activation_flow() {
    let bind_addr = reserve_local_bind_addr();
...
#[test]
fn device_edge_bridge_metrics_reflect_activation_registry_backend_after_activation_flow() {
    let bind_addr = reserve_local_bind_addr();
    let _edge_runtime = DeviceEdgeProcess::spawn_with_env(
        &bind_addr,
        &[
...
#[test]
fn device_edge_activation_registry_stats_record_consume_miss_on_replay_attempt() {
    let bind_addr = reserve_local_bind_addr();
    let _edge_runtime = DeviceEdgeProcess::spawn_with_env(
        &bind_addr,
        &[
...
#[test]
fn device_edge_redis_registry_kind_without_url_falls_back_to_in_memory_backend() {
    let bind_addr = reserve_local_bind_addr();
    let _edge_runtime = DeviceEdgeProcess::spawn_with_env(
        &bind_addr,
        &[
...
#[test]
fn device_edge_redis_registry_round_trip_when_test_redis_url_is_configured() {
    let Some(redis_url) = std::env::var("SDKWORK_AIOT_DEVICE_EDGE_TEST_REDIS_URL").ok() else {
        return;
    };
    let bind_addr = reserve_local_bind_addr();
    let _edge_runtime = DeviceEdgeProcess::spawn_with_env(
...
#[test]
fn device_edge_accepts_new_http_requests_while_xiaozhi_websocket_is_open() {
    let bind_addr = reserve_local_bind_addr();
    let _edge_runtime = DeviceEdgeProcess::spawn(&bind_addr);
    ...
    let mut http = TcpStream::connect(&bind_addr).expect("readiness tcp connection");
    http.set_read_timeout(Some(Duration::from_secs(2)))
        .expect("http timeout");
    http.write_all(b"GET /readyz HTTP/1.1\r\nHost: local\r\n\r\n")
        .expect("readiness request write");
...
fn http_get_or_post(bind_addr: &str, request: &[u8]) -> String {
    let mut http = TcpStream::connect(bind_addr).expect("device edge TCP connection");
    http.set_read_timeout(Some(Duration::from_secs(2)))
        .expect("http timeout");
    http.write_all(request).expect("device edge request write");
    read_http_response(&mut http)
}

const DEVICE_EDGE_SPAWN_ENV_ISOLATION_KEYS: &[&str] = &[
    "SDKWORK_AIOT_XIAOZHI_ACTIVATION_CHALLENGE",
    "SDKWORK_AIOT_XIAOZHI_ACTIVATE_EXPECTED_CHALLENGE",
    "SDKWORK_AIOT_XIAOZHI_ACTIVATE_EXPECTED_HMAC",
...
fn spawn_device_edge_with_env(bind_addr: &str, env_overrides: &[(&str, Option<&str>)]) -> Child {
    ...
    let mut command = Command::new(device_edge_binary());
    command
        .env("SDKWORK_AIOT_EDGE_DEVICE_INGRESS_BIND", bind_addr)
        .env("SDKWORK_AIOT_DEV_MODE", "1")
        .env_remove("SDKWORK_AIOT_DEVICE_EDGE_NO_LISTEN")
        .env_remove("SDKWORK_AIOT_DEVICE_EDGE_MQTT_BRIDGE_ENABLE")
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    for key in DEVICE_EDGE_SPAWN_ENV_ISOLATION_KEYS {
    ...
    command
        .spawn()
        .expect("spawn sdkwork-aiot-device-edge-runtime")
}

fn spawn_device_edge(bind_addr: &str) -> Child {
    spawn_device_edge_with_env(bind_addr, &[])
}

struct DeviceEdgeProcess {
    child: Child,
}

impl DeviceEdgeProcess {
    fn spawn(bind_addr: &str) -> Self {
        Self {
            child: spawn_device_edge(bind_addr),
        }
    }

    fn spawn_with_env(bind_addr: &str, env_overrides: &[(&str, Option<&str>)]) -> Self {
        Self {
            child: spawn_device_edge_with_env(bind_addr, env_overrides),
        }
    }
}

impl Drop for DeviceEdgeProcess {
    fn drop(&mut self) {
...
fn device_edge_binary() -> String {
    option_env!("CARGO_BIN_EXE_sdkwork-aiot-device-edge-runtime")
        .map(str::to_string)
        .unwrap_or_else(|| "target/debug/sdkwork-aiot-device-edge-runtime.exe".to_string())
}

fn wait_for_device_edge(bind_addr: &str, timeout: Duration) {
    ...
    loop {
        ...
        assert!(
            ...
            "device edge runtime did not start"
        );
...
fn xiaozhi_device_token_valid_accepts_sqlite_stored_credentials_in_production_mode() {
    ...
    let authorized = parse_http_request_bytes(
        format!(
            "GET /iot/xiaozhi/ws?device_id=edge-device-001&client_id=client-001 HTTP/1.1\r\nHost: domain\r\nAuthorization: Bearer {issued_secret}\r\n\r\n"
        )
        .as_bytes(),
    )
    ...
    let wrong_token = parse_http_request_bytes(
        b"GET /iot/xiaozhi/ws?device_id=edge-device-001&client_id=client-001 HTTP/1.1\r\nHost: domain\r\nAuthorization: Bearer wrong-token\r\n\r\n",
    )
...
fn rollout_aware_ota_provider_delivers_firmware_once_per_pending_deployment() {
    ...
    let server = standard_device_edge_server_with_plugins(
...
fn internal_route_requires_token_when_dev_mode_disabled() {
    let _env = EnvGuard::set_all_locked(&[
        ...
        (
            ...
            Some("device-edge-production-token-32chars"),
        ),
    ]);
    ...
    assert!(!sdkwork_aiot_device_edge_runtime::internal_route_authorized(&missing));

    let authorized = parse_http_request_bytes(
        b"GET /internal/bridge/health HTTP/1.1\r\nHost: local\r\nx-sdkwork-internal-token: device-edge-production-token-32chars\r\n\r\n",
    )
    .expect("authorized request");
    assert!(sdkwork_aiot_device_edge_runtime::internal_route_authorized(
        &authorized
    ));
}
